...
interface ZaloBadgeProps {
  className?: string;
  size?: 'sm' | 'md' | 'lg';
}

/**
 * Zalo Avatar Badge tròn với nền gradient xanh dương chuẩn nhận diện thương hiệu Zalo
 */
export const ZaloBadge: React.FC<ZaloBadgeProps> = ({ className = '', size = 'md' }) => {
  const sizeClasses = {
    sm: 'w-6 h-6 p-1 text-[9px]',
    md: 'w-8 h-8 p-1.5 text-xs',
    lg: 'w-11 h-11 p-2 text-sm',
  }[size];

  return (
    <div
      className={`rounded-2xl bg-gradient-to-tr from-[#0068FF] to-[#0091FF] text-white flex items-center justify-center shadow-md shadow-blue-500/25 shrink-0 ${sizeClasses} ${className}`}
    >
