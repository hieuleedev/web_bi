      ...
      try {
        ...
        if (data && data.length > 0) {
          const mappedOrders: Order[] = data.map((row: any) => ({
            ...
            shippingFee: row.shippingFee ?? row.shipping_fee ?? 0,
            serviceFee: 0,
            totalAmount: row.totalAmount ?? ((row.total_rent_fee || 0) + (row.total_buy_price || 0) + (row.total_deposit || 0) + (row.shipping_fee || 0)),
